 ...
 * @brief Defines a simple test model, a variation of The Cornel Box.
 *
 */
...
/**
 * @brief Loads the test model by filling the given vectors.
 *
 * -1 <= x <= +1
 * -1 <= y <= +1
 * -1 <= z <= +1
...
void LoadTestModel(std::vector<Triangle> &triangles, std::vector<Sphere> &spheres)
{
	...
	// Define the sphere
	spheres.push_back(Sphere(vec3(130, 90, 130), 90));
